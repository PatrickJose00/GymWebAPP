...
class PlanonutricaoController extends Controller {
    ...
    public function actionCriarPdf($id) {
        ...
        try {
            $pdf->render();
            return $this->redirect(['perfil/historico-plano-nutricao']);
        } catch (\Exception $e) {
            return $this->redirect(['perfil/historico-plano-nutricao']);
        }
    }
    ...
    public function checkUser($model) {
        if (!Yii::$app->user->isGuest) {
            ...
            if ($cliente !== null) {
                $checkPlano = PlanosNutricaoCliente::find()->where(['Clientes_id' => $cliente->id])->all();
                foreach ($checkPlano as $check) {
                    if ($model->id == $check->PlanosdeNutricao_id) {
                        return true;
                    }
                }
                return false;
